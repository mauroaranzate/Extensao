...
dados_sinasc_2$SEXO_AUX <- ifelse(dados_sinasc_2$SEXO=="Masculino","M","F")

dados_sinasc_2 <- merge(dados_sinasc_2, tabela_pig,
                        by.x=c("SEMAGESTAC","SEXO_AUX"),
                        by.y=c("SEMAGESTAC","SEXO"),
                        all.x=TRUE)
...
dados_sinasc_2$MUNIC <- dados_sinasc_2$CODMUNRES

TN <- aggregate(CONTADOR ~ MUNIC, dados_sinasc_2, length)
names(TN)[2] <- "TN"

TGI_15_19 <- aggregate(IDADEMAE ~ MUNIC, dados_sinasc_2,
                       function(x) sum(x>=15 & x<=19, na.rm=TRUE))
names(TGI_15_19)[2] <- "TGI_15_19"

lista <- list(TN, TGI_15_19)

base_final <- Reduce(function(x,y) merge(x,y,by="MUNIC",all=TRUE), lista)

uf <- base_final[1,]
uf[1,] <- NA
uf$MUNIC <- "42"
uf$TN <- nrow(dados_sinasc_2)

SINASC_SC <- rbind(uf, base_final)
